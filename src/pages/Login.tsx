...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!rememberMe) {
        // The token is already stored in localStorage by the login function
        // This is just to indicate we should clear it on browser close
        sessionStorage.setItem('wheelshare_remember', 'false');
      } else {
        sessionStorage.setItem('wheelshare_remember', 'true');
      }
      ...
      <div className="w-full max-w-md">
        <Card>
          <CardHeader className="space-y-1">
            <CardTitle className="text-2xl font-bold text-center">Login to WheelShare</CardTitle>
